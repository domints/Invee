...
/// Returns `true` via pop if the storage was created successfully.
class AddStorageScreen extends StatefulWidget {
  final ApiService apiService;

  /// When provided, the new storage will be pre-filled as a child of this.
  final int? parentId;
...
class _AddStorageScreenState extends State<AddStorageScreen> {
  ...
  List<StorageTypeDto>? _storageTypes;
  List<PickerEntry>? _storageEntries;
  bool _loadingData = true;
  String? _loadError;

  StorageTypeDto? _selectedType;
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await Future.wait([
        widget.apiService.getStorageTypes(),
        widget.apiService.getStorageTree(),
      ]);
      if (!mounted) return;
      final types = results[0] as List<StorageTypeDto>;
      final tree = results[1] as List<StorageTreeResponse>;
      final entries = PickerEntry.fromStorageTree(tree);
      setState(() {
        _storageTypes = types;
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to create storage: $e'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    }
        ...
        children: [
          TextFormField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: 'e.g. Living Room Cabinet',
          ...
          DropdownButtonFormField<StorageTypeDto>(
            ...
            validator: (v) =>
                v == null ? 'Please select a storage type' : null,
          ),
          const SizedBox(height: 16),
          // Parent picker
          InkWell(
            ...
            child: InputDecorator(
              decoration: InputDecoration(
                labelText: 'Parent storage (optional)',
              ...
              child: Text(
                _selectedParent?.label ?? 'None (root storage)',
          ...
          FilledButton.icon(
            ...
            label: const Text('Create Storage'),
          ),
        ],
      ),
    );

...
{
    public class GetCategoryItemsHandler : IRequestHandler<GetCategoryItems, OperationResult<List<ItemListEntry>>>
    {
        private readonly InveeContext _db;

        public GetCategoryItemsHandler(InveeContext db)
        {
            _db = db;
        }

        public async Task<OperationResult<List<ItemListEntry>>> Handle(GetCategoryItems request, CancellationToken cancellationToken)
        {
            var items = await _db.Items.Where(i => i.CategoryId == request.Id).OrderBy(i => i.Name).Select(ItemConverter.ToListEntryExpr).ToListAsync(cancellationToken);
